  ...
  <div class="process-page">
    ...
    <nav class="navbar">
      ...
      <div class="nav-center">
        ...
        <div class="step-name">{{ $t('nav.graphBuild') }}</div>
      </div>
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">{{ $t('graph.liveGraph') }}</span>
          </div>
          <div class="header-right">
            <template v-if="graphData">
              <span class="stat-item">{{ graphData.node_count || graphData.nodes?.length || 0 }} {{ $t('graph.nodes') }}</span>
              <span class="stat-divider">|</span>
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} {{ $t('graph.relations') }}</span>
              <span class="stat-divider">|</span>
            </template>
            <div class="action-buttons">
                <button class="action-btn" @click="refreshGraph" :disabled="graphLoading" :title="$t('graph.refresh')">
                  <span class="icon-refresh" :class="{ 'spinning': graphLoading }">↻</span>
                </button>
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? $t('graph.exitFullscreen') : $t('graph.fullscreen')">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">{{ $t('graph.graphLoading') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">{{ $t('graph.autoStartBuild') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">{{ $t('graph.graphBuilding') }}</p>
            <p class="waiting-hint">{{ $t('graph.dataComingSoon') }}</p>
          </div>
        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('graph.ontology') }}</div>
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">{{ $t('process.progressLabel') }}</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('process.entityTypesLabel') }} ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('process.relationTypesLabel') }} ({{ projectData.ontology.relation_types?.length || 0 }})</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('nav.graphBuild') }}</div>
              ...
              <div class="detail-section" v-if="graphData">
                ...
                <div class="build-result">
                  ...
                  <div class="result-item">
                    ...
                    <span class="result-label">{{ $t('graph.relationEdges') }}</span>
                  </div>
                  <div class="result-item">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('graph.buildComplete') }}</div>
                <div class="phase-api">{{ $t('process.prepareNext') }}</div>
              </div>
        ...
        <div class="project-panel">
          ...
          <div class="project-details" v-if="projectData">
            ...
            <div class="project-item" v-if="projectData.graph_id">
              <span class="item-label">{{ $t('process.graphIdLabel') }}</span>
...
const statusText = computed(() => {
  if (error.value) return t('status.buildFailed')
  if (currentPhase.value >= 2) return t('status.buildComplete')
  if (currentPhase.value === 1) return t('status.graphBuilding')
  if (currentPhase.value === 0) return t('status.ontologyGenerating')
...
const getPhaseStatusText = (phase) => {
  if (currentPhase.value > phase) return t('status.completed')
...
const handleNewProject = async () => {
  ...
  try {
    ...
    error.value = t('process.initFailed', { error: err.message || 'unknown' })
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || t('process.loadFailed', { error: '' })
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = t('process.loadFailed', { error: err.message || 'unknown' })
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = t('process.buildTaskStarted')
      ...
      error.value = response.error || t('process.startBuildFailed', { error: '' })
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = t('process.startBuildFailed', { error: err.message || 'unknown' })
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      buildProgress.value = {
        ...
        message: task.message || t('status.processing')
      }
      ...
      if (task.status === 'completed') {
        ...
        stopPolling()
        stopGraphPolling()
        error.value = t('process.graphBuildFailed', { error: task.error || 'unknown' })
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  const edges = edgesData
    ...
    .map(e => ({
      ...
      rawData: {
        ...e,
        source_name: nodeMap[e.source_node_uuid]?.name || t('graph.unknown'),
        target_name: nodeMap[e.target_node_uuid]?.name || t('graph.unknown')
      }
    }))
