...
class SimulationConfigGenerator:
    ...
    def _generate_time_config(self, context: str, num_entities: int) -> Dict[str, Any]:
        ...
        prompt = get_prompt('sim_config', 'time_user').format(
            context_truncated=context_truncated,
            max_agents_allowed=max_agents_allowed
        )

        system_prompt = get_prompt('sim_config', 'time_system')
    ...
    def _generate_event_config(
        ...
        prompt = get_prompt('sim_config', 'event_user').format(
            simulation_requirement=simulation_requirement,
            context_truncated=context_truncated,
            type_info=type_info
        )

        system_prompt = get_prompt('sim_config', 'event_system')
    ...
    def _generate_agent_configs_batch(
        ...
        prompt = get_prompt('sim_config', 'agent_user').format(
            simulation_requirement=simulation_requirement,
            entity_list_json=json.dumps(entity_list, ensure_ascii=False, indent=2)
        )

        system_prompt = get_prompt('sim_config', 'agent_system')
